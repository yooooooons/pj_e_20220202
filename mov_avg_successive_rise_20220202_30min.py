...
candle_type = '30min'   # 사용하는 candle 종류 (1min, 3min, 5min, 10min, 15min, 30min, 60min)
...
#moving_avg_value_criteria_init = 0.9   # 최근 이동평균선 1 변화비율 x 최근 이동평균선 2 변화비율
moving_avg_value_criteria = 1.0001   # 최근 이동평균선 1 변화비율 x 최근 이동평균선 2 변화비율

invest_ratio = 0.018   # 보유 금액의 최대 몇 % 를 투자할것인가 (예> 0.1 <-- 보유금액 10% 투자)
...
sell_auto = 0.01   # 매수가 대비 몇 % 이상 하락했을때, candle 추이와 관계없이 자동 매도 하겠는가? (예> 0.02 <--- 2% 이상 상승시 자동 매도
...
qualified_coin_day_count = int((2/3) * 24 * (60/time_unit)) + 1   #가장 최근 시간대를 제외해야 하기 때문에  + 1을 함
...
factor_ratio_3 = 0.1
factor_ratio_4 = 0.25
factor_ratio_5 = 1 - (factor_ratio_1 + factor_ratio_2 + factor_ratio_3 + factor_ratio_4)

open_price_ratio_configure_ratio = 1
volume_price_configure_ratio = 0.7
No_of_rise_configure_ratio = 1
...
def qualified_coin () :
    ...
    DF_candidate['potential_factor'] = (factor_ratio_1 * (DF_candidate['open_price_ratio'] / DF_basic_info_selected['open_price_ratio'].mean()))     + (factor_ratio_2 * (DF_candidate['volume_price'] / DF_basic_for_volume_check['volume_price'].mean()))     + (factor_ratio_3 * (DF_candidate['No_of_rise'] / DF_basic_info_selected['No_of_rise'].mean()))     + (factor_ratio_4 * DF_candidate['moving_avg_value_1']) + (factor_ratio_5 * DF_candidate['moving_avg_value_2'])
...
bought_coin = 'KRW-BTC'
bought_opt_successive_rise_No = 10
bought_dec_acept_No = 7
check_coin_currency = 'BTC'
